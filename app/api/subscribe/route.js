export async function POST(request) {
  try {
    ...
    const res = await fetch(
      ...
      { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ values: [[email, new Date().toISOString()]] }) }
    )
...
async function signJWT(payload, privateKeyPem) {
  ...
  const keyData = privateKeyPem.replace(/-----BEGIN PRIVATE KEY-----/g,'').replace(/-----END PRIVATE KEY-----/g,'').replace(/\s+/g,'').trim()
